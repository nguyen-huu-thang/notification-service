...
**Gửi bền vững (outbox)**
- Lưu mọi email (bảng `email_notifications`) để retry, audit và idempotency
- Tự động retry với exponential backoff khi lỗi tạm thời; hết lượt thì dead-letter
- `idempotency_key` tùy chọn do caller cung cấp để tránh gửi trùng
- Worker nền: gửi lại các notification đến hạn và dọn dữ liệu cũ theo retention
...
Các đối tượng domain (`EmailNotification`) là class Python thường, kiểm tra invariant
trong constructor và expose state qua property chỉ-đọc. Chúng immutable: thay đổi trạng
thái (`mark_sent`, `schedule_retry`, `dead_letter`) trả về instance mới thay vì mutate tại
chỗ. ID dùng value object `Id` (KSUID, 24 byte), không dùng `bytes` thuần.
...
├── scheduler/    ← Job nền (retry email, dọn dữ liệu retention)
├── integration/  ← Bootstrap mTLS + đồng bộ cert/key với Trust Service
├── config/       ← Cấu hình DI, gRPC, scheduler
└── common/       ← Constant, exception, utility
```

Domain layer không phụ thuộc vào infrastructure, framework hay database. Mọi dependency đều hướng vào trong. Persistence dùng PostgreSQL + Alembic (bảng: `trust_*`, `email_notifications`).
