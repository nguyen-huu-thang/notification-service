...
| Nếu SMTP bị down? | Email đã lưu (PENDING/FAILED); worker tự gửi lại với backoff, hết lượt thì dead-letter |
| Có trạng thái không? | Có — PostgreSQL lưu outbox email + cert/key mTLS |
...
- Không phải message broker — không nhận event bất đồng bộ (gọi qua gRPC trực tiếp); tuy có outbox + retry nội bộ nhưng không phải hệ thống hàng đợi đầy đủ
